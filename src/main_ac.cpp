...
void initializeDrivers(ros::NodeHandle& nh, const ros::NodeHandle& nh_priv) {
    ROS_ERROR("getting into initializeDrivers");
    ...
    try {
        std::shared_ptr<rebot_driver> rd = std::make_shared<rebot_driver>(
            nh, curtis_can_tx_topic, car_rpm_max, servo_zero_point,
            curtis_theta_oppsite, curtis_vel_oppsite, Curtis_Steer_Enable, near_threshold);
        ac.push_back(rd);
        ROS_INFO("Rebot driver successfully created and added.");
    } catch (const std::exception& e) {
        ROS_ERROR_STREAM("Exception caught during rebot_driver initialization: " << e.what());
        // 添加更多的日志记录或处理代码
    }
...
void servo_cmd_callback(const common::servo_cmd::ConstPtr &msg)
{
    ...
    ROS_ERROR_STREAM("ac.size(): "<< ac.size());
...
void get_maual_auto(const common::peripheral_uart::ConstPtr &msg)
{
    ...
    */
    ...
    //     // for (int i = 0; i < ac.size(); i++)
    //     // {
    //     //     ROS_ERROR_STREAM("ac init...." << i << " ac.size(): " << ac.size());
    //     //     // ac[i]->init();
    //     // }
    //     // cd->init();
    // }
    ...
    // for(int i = 0; i < ac.size(); i++)
    // {
    //     ROS_ERROR_STREAM("set_manual_mode...." << i << " ac.size(): " << ac.size());
