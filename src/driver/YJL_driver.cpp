...
YJL_driver::YJL_driver(ros::NodeHandle &nh,
...
YJL_driver::~YJL_driver()
{
}

void YJL_driver::set_zero_point(double servo_zero_point)
{
    servo_zero_point_ = servo_zero_point;
    if (theta_oppsite_)
        servo_zero_point_ = -servo_zero_point_;
}
bool YJL_driver::judge_angle_near(const common::servo_cmd::ConstPtr &msg, common::servo_encoder se)
{
...
void YJL_driver::set_servo_cmd(const common::servo_cmd::ConstPtr &msg, bool syn_enable)
{
...
// theta :绝对角度，单位度
// vel：运行速度，无符号，单位转每分钟
void YJL_driver::send_servo_cmd(common::servo_cmd &sc)
{
    ...
    if (sc_speed > 0)
    {
        can_s.datas[0] = 0x0d; // 前进
        can_s.datas[1] = (sc_angle >> 8) & 0x00ff;
        can_s.datas[2] = sc_angle & 0x00ff;
        can_s.datas[3] = uint8_t((sc_speed >> 8) & 0x00ff);
        can_s.datas[4] = uint8_t(sc_speed & 0x00ff);
        can_s.datas[5] = 0;
        can_s.datas[6] = 0;
        can_s.datas[7] = 0x0f;
    }
    else if (sc_speed < 0)
    {
        ...
        can_s.datas[1] = (sc_angle >> 8) & 0x00ff;
        can_s.datas[2] = sc_angle & 0x00ff;
        can_s.datas[3] = uint8_t((sc_speed_ll >> 8) & 0x00ff);
        can_s.datas[4] = uint8_t(sc_speed_ll & 0x00ff);
        can_s.datas[5] = 0;
        can_s.datas[6] = 0;
        can_s.datas[7] = 0x0f;
    }
...
int YJL_driver::controller_error_report(const common::can::ConstPtr &msg, std::string &error_msg_s)
{
    return 0;
}

// 收到车体的反馈，解析舵轮的角度和线速度
int YJL_driver::controller_state_report(const common::can::ConstPtr &msg, common::servo_encoder &se, std::string &error_msg_s)
{
    last_recv_time_ = ros::Time::now().toSec();
...
int YJL_driver::timer_can_send(void)
{
...
bool YJL_driver::need_reset(void)
{
    return false;
}

void YJL_driver::set_manual_mode(int manual_mode)
{
    manual_mode_ = manual_mode;
}

void YJL_driver::init(void)
{
    init_cnt_ = 60;
}

void YJL_driver::disable_send(void)
{
    send_enable_ = false;
}

void YJL_driver::enable_send(void)
{
    send_enable_ = true;
}
